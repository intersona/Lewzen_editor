...
import { genRandomStr,boxWidth } from '../../utils/util'
...
function genDomID(): string {
  return genRandomStr('insert-formula')
}

class EditFormulaMenu implements IModalMenu {
  readonly title = t('编辑公式')
  readonly iconSvg = PENCIL_SVG
  readonly tag = 'button'
  readonly showModal = true // 点击 button 时显示 modal
  readonly modalWidth = boxWidth
  private $content: Dom7Array | null = null
  private readonly inputId = genDomID()
  ...
  getModalContentElem(editor: IDomEditor): DOMElement {
    const { inputId, buttonId } = this


      var inputContainerElem = document.createElement('lable')
      const containerTitleSpan = document.createElement('span')
      const titleText = document.createTextNode('LaTeX公式')
      containerTitleSpan.appendChild(titleText)
      inputContainerElem.appendChild(containerTitleSpan)
      var inputElem = document.createElement('input')
      inputElem.setAttribute('placeholder','使用LaTeX语法')
    ...
    buttonElem.setAttribute('id',buttonId)
    const buttonText = document.createTextNode('确定')
    ...
    if (this.$content == null) {
      ...
      $content.on('click', `#${buttonId}`, e => {
        e.preventDefault()
        const value = $content.find(`#${inputId}`).val().trim()
    ...
    $content.append(inputContainerElem)
